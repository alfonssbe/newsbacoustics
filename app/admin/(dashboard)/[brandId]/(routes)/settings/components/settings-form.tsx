...
export const SettingsForm: React.FC<SettingsFormProps> = ({
  ...
  const onSubmit = async (data: SettingsFormValues) => {
    try {
      setLoading(true);
      const response = await axios.patch(`${process.env.NEXT_PUBLIC_ADMIN_FOLDER_URL}/api/brands/${params.brandId}`, data);
  ...
  const changeEverything = async () => {
    try {
      setLoading(true);
      const response = await axios.post(`${process.env.NEXT_PUBLIC_ADMIN_FOLDER_URL}/api/temp`, '');
      if(response.data === 'expired_session'){
        router.push(`${process.env.NEXT_PUBLIC_ADMIN_FOLDER_URL}/`);
        router.refresh();
        toast.error("Session expired, please login again")
  ...
  return (
    ...
    />
     <div className="flex items-center justify-between">
      ...
      />
      <Button className="ml-auto" type="submit" onClick={changeEverything}>
        Change database
      </Button>
    </>
  );
