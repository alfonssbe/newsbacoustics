...
export default function AllDriversPageClient() { 
    ...
    const fetchData = async () => {
      try {
        if (searchParams.has("search")) {
          router.replace(pathname); // removes ?search from URL
        }
        const storage = localStorage.getItem("allDriversProducts") ?? "";
        document.body.style.overflow = 'hidden';
        let tempData = await getAllProductsBySlug(pathname, storage);
        // tempData.allproduct.allProducts.map((val) => val.name.toLowerCase().includes('kapton') && console.log(val ) )
        ...
        let counterShow = 0;
          ...
          tempCheckboxLoop.push(
            createFilterProps('allSubCategory', 'Type', '', 'all_sub_category'),
            createFilterProps('allMaterial', 'Material', '', 'all_material'),
          )
