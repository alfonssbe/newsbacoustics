    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                let tempdatasheetCatalogues = await getMultipleDatasheetProduct(productSlug) ?? []
                let temp3dmodels = await getMultiplemodels3DProduct(productSlug) ?? []
                let tempfrdzmafiles = await getMultipleFRDZMA(productSlug) ?? []
                let tempsimilarprod = await getSimilar(productSlug)
                ...
                if (tempdata.isCustom) {
                    tempcustomProps = await getCustomProduct(productSlug);
                    ...
                    if(tempdata.isKits){
                        tempkitsFinishing = await getKitsFinishingProduct(productSlug)
                    }
